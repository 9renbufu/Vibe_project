...
class DrawingAgent:
    ...
    async def correct_command(self, raw_text: str, confidence: float) -> str:
        """语音指令纠错：当 parser 置信度低时，用 LLM 理解意图"""
        if not self.llm:
            return raw_text

        prompt = f"""用户语音输入: "{raw_text}"
当前解析置信度: {confidence:.0%}

请将用户的语音指令纠正为标准绘图指令。规则：
1. 修正错别字和同音字
2. 补全省略的量词和动词
3. 如果是抽象描述（如"画一个温暖的感觉"），转换为具体的绘图指令
4. 只返回纠正后的指令文本，不要解释

支持的绘图类型：
- 基础图形：画一个圆、画矩形、画三角形、画星形、画心形、画椭圆、画线条
- 程序化艺术：画流场、画分形树、画水彩、画曼陀罗、画螺线、画沃罗诺伊、画粒子、画波浪、画条纹、画渐变
- 风景场景：画日落、画海洋、画山脉、画星空、画森林、画草原、画沙漠、画雪景、画春天
- 颜色：红色、蓝色、绿色、黄色、紫色、橙色、粉色、黑色、白色、灰色、金色、棕色、青色
- 位置：中间、左上角、右上角、左下角、右下角、上面、下面、左边、右边
- 大小：大、中、小
- 操作：撤销、重做、清空"""

        system = "你是绘图指令纠错助手。用户通过语音输入绘图指令，语音识别可能有错别字。请将输入纠正为标准绘图指令。只返回纠正后的文本，不要解释。"
